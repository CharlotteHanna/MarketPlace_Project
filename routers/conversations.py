...
from schemas import ConversationBase, ConversationDisplay, UserBase
...
def start_conversation(request: ConversationBase, db: Session = Depends(get_db), current_user: UserBase = Depends(get_current_user)):
...
def get_all_conversations(db: Session = Depends(get_db), current_user: UserBase = Depends(get_current_user)):
...
def get_conversation(id: int, db: Session = Depends(get_db), current_user: UserBase = Depends(get_current_user)):
  return db_conversations.get_conversation_by_id(db,id) 
  
#Use Delete Conversation functionality from db_conversations file
@router.delete('/{id}')
def delete_conversation(id: int, db: Session = Depends(get_db), current_user: UserBase = Depends(get_current_user)):
