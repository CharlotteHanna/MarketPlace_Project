...
def create_payment(request: PaymentsBase, db: Session = Depends(get_db), current_user: UserBase = Depends(get_current_user)):
...
def get_all_payments(db: Session = Depends(get_db), current_user: UserBase = Depends(get_current_user)):
    return db_payments.get_all_payments(db)

    #return payments with conditions
@router.get('/{id}', response_model=PaymentsDisplay)
def get_payment(id: int, db: Session = Depends(get_db), current_user: UserBase = Depends(get_current_user)):
    return db_payments.get_payment(db, id)

#Use Update Payment functionality from db_payments file
@router.post('/{id}')
def update_payment(id: int,request: PaymentsBase, db: Session = Depends(get_db), current_user: UserBase = Depends(get_current_user)):
    return db_payments.update_payment(db, id, request)

#Use Delete Payment functionality from db_payments file
@router.delete('/{id}')
def delete_payment(id: int, db: Session = Depends(get_db), current_user: UserBase = Depends(get_current_user)):
